...
from .softnote import opcodes, make_op_return, address_to_scripthash
...
from .softnote import redeem_script_to_address
...
class LNWorker(Logger, NetworkRetryManager[LNPeerAddr]):
    ...
    async def _get_next_peers_to_try(self) -> Sequence[LNPeerAddr]:
        ...
        if constants.net in (constants.SoftnoteTestnet,):
            fallback_list = FALLBACK_NODE_LIST_TESTNET
        elif constants.net in (constants.SoftnoteMainnet,):
...
class LNWallet(LNWorker):
    ...
    async def create_routes_for_payment(
        ...
        try:
            ...
            use_singe_node = not self.channel_db and constants.net is constants.SoftnoteMainnet
    ...
    async def create_invoice(
        ...
        amount_sfn = amount_msat/Decimal(COIN*1000) if amount_msat else None
        if expiry == 0:
    ...
    def current_feerate_per_kw(self):
        ...
        if constants.net is constants.SoftnoteRegtest:
