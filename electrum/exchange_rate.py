...
DEFAULT_EXCHANGE = "BlockchainInfo"  # default exchange should ideally provide historical rates
...
class FxThread(ThreadJob):
    ...
    def format_amount(self, sfn_balance, *, timestamp: int = None) -> str:
        if timestamp is None:
            rate = self.exchange_rate()
        else:
            rate = self.timestamp_rate(timestamp)
        return '' if rate.is_nan() else "%s" % self.value_str(sfn_balance, rate)

    def format_amount_and_units(self, sfn_balance, *, timestamp: int = None) -> str:
        if timestamp is None:
            rate = self.exchange_rate()
        else:
            rate = self.timestamp_rate(timestamp)
        return '' if rate.is_nan() else "%s %s" % (self.value_str(sfn_balance, rate), self.ccy)

    def get_fiat_status_text(self, sfn_balance, base_unit, decimal_point):
        rate = self.exchange_rate()
