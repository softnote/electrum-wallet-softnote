...
from .softnote import make_op_return
...
class Peer(Logger):
    ...
    def on_announcement_signatures(self, chan: Channel, payload):
        if chan.config[LOCAL].was_announced:
            h, local_node_sig, local_softnote_sig = self.send_announcement_signatures(chan)
        else:
    ...
    def make_local_config(self, funding_sat: int, push_msat: int, initiator: HTLCOwner) -> LocalConfig:
        ...
        if self.is_static_remotekey():
            ...
            static_remotekey = None
        dust_limit_sat = softnote.DUST_LIMIT_DEFAULT_SAT_LEGACY
    ...
    async def channel_establishment_flow(
        ...
        funding_address = softnote.redeem_script_to_address('p2wsh', redeem_script)
    ...
    async def handle_announcements(self, chan: Channel):
        ...
        remote_softnote_sig = announcement_signatures_msg["softnote_signature"]
        if not ecc.verify_signature(chan.config[REMOTE].multisig_key.pubkey, remote_softnote_sig, h):
            raise Exception("softnote_sig invalid in announcement_signatures")
        if not ecc.verify_signature(self.pubkey, remote_node_sig, h):
            raise Exception("node_sig invalid in announcement_signatures")

        node_sigs = [remote_node_sig, local_node_sig]
        softnote_sigs = [remote_softnote_sig, local_softnote_sig]
        softnote_keys = [chan.config[REMOTE].multisig_key.pubkey, chan.config[LOCAL].multisig_key.pubkey]

        if self.node_ids[0] > self.node_ids[1]:
            node_sigs.reverse()
            softnote_sigs.reverse()
            node_ids = list(reversed(self.node_ids))
            softnote_keys.reverse()
        else:
        ...
        self.send_message("channel_announcement",
            ...
            softnote_signature_1=softnote_sigs[0],
            softnote_signature_2=softnote_sigs[1],
            len=0,
            ...
            softnote_key_1=softnote_keys[0],
            softnote_key_2=softnote_keys[1]
        )
    ...
    def send_announcement_signatures(self, chan: Channel):
        ...
        softnote_signature = ecc.ECPrivkey(chan.config[LOCAL].multisig_key.privkey).sign(msg_hash, sig_string_from_r_and_s)
        ...
        self.send_message("announcement_signatures",
            ...
            softnote_signature=softnote_signature
        )
        return msg_hash, node_signature, softnote_signature
    ...
    def on_update_add_htlc(self, chan: Channel, payload):
        ...
        if cltv_expiry > softnote.NLOCKTIME_BLOCKHEIGHT_MAX:
            asyncio.ensure_future(self.lnworker.try_force_closing(chan.channel_id))
    ...
    async def maybe_update_fee(self, chan: Channel):
        ...
        if not chan.constraints.is_initiator:
            if constants.net is not constants.SoftnoteRegtest:
    ...
    async def send_shutdown(self, chan: Channel):
        ...
        if chan.config[LOCAL].upfront_shutdown_script:
            ...
            scriptpubkey = bfh(softnote.address_to_script(chan.sweep_address))
        assert scriptpubkey
    ...
    async def _shutdown(self, chan: Channel, payload, *, is_local: bool):
        ...
        if chan.config[LOCAL].upfront_shutdown_script:
            ...
            our_scriptpubkey = bfh(softnote.address_to_script(chan.sweep_address))
