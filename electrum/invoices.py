...
class OnchainInvoice(Invoice):
    ...
    def _validate_amount(self, attribute, value):
        if isinstance(value, int):
            if not (0 <= value <= TOTAL_COIN_SUPPLY_LIMIT_IN_SFN * COIN):
...
class LNInvoice(Invoice):
    ...
    def _validate_amount(self, attribute, value):
        if value is None:
            return
        if isinstance(value, int):
            if not (0 <= value <= TOTAL_COIN_SUPPLY_LIMIT_IN_SFN * COIN * 1000):
    ...
    def get_amount_msat(self) -> Optional[int]:
        amount_sfn = self._lnaddr.amount
        amount = int(amount_sfn * COIN * 1000) if amount_sfn else None
    ...
    def to_debug_json(self) -> Dict[str, Any]:
        ...
        d.update({
            ...
            'amount_SFN': str(self._lnaddr.amount),
