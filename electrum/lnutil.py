...
from . import ecc, softnote, crypto, transaction
from .softnote import (push_script, redeem_script_to_address, address_to_script,
...
class Config(StoredObject):
    ...
    def validate_params(self, *, funding_sat: int) -> None:
        ...
        if self.dust_limit_sat < softnote.DUST_LIMIT_DEFAULT_SAT_LEGACY:
...
def make_htlc_tx_output(amount_msat, local_feerate, revocationpubkey, local_delayedpubkey, success, to_self_delay):
    ...
    p2wsh = softnote.redeem_script_to_address('p2wsh', bh2u(script))
...
def make_offered_htlc(revocation_pubkey: bytes, remote_htlcpubkey: bytes,
    ...
    script = bfh(construct_script([
        opcodes.OP_DUP,
        opcodes.OP_HASH160,
        softnote.hash_160(revocation_pubkey),
        opcodes.OP_EQUAL,
        opcodes.OP_IF,
        opcodes.OP_CHECKSIG,
        opcodes.OP_ELSE,
...
def make_commitment_outputs(*, fees_per_participant: Mapping[HTLCOwner, int], local_amount_msat: int, remote_amount_msat: int,
    ...
    for script, htlc in htlcs:
        addr = softnote.redeem_script_to_address('p2wsh', bh2u(script))
...
def make_commitment_output_to_local_address(
    ...
    return softnote.redeem_script_to_address('p2wsh', bh2u(local_script))

def make_commitment_output_to_remote_address(remote_payment_pubkey: bytes) -> str:
    return softnote.pubkey_to_address('p2wpkh', bh2u(remote_payment_pubkey))
