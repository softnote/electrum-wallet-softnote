...
from electrum.softnote import OnchainOutputType

import electrum.softnote as softnote
...
class BitBox02Client(HardwareClientBase):
    ...
    def coin_network_from_electrum_network(self) -> int:
        if constants.net.TESTNET:
            return bitbox02.sfn.TSFN
        return bitbox02.sfn.SFN
    ...
    def get_password_for_storage_encryption(self) -> str:
        ...
        node = bip32.BIP32Node.from_xkey(xpub, net = constants.SoftnoteMainnet()).subkey_at_public_derivation(())
    ...
    def get_xpub(self, bip32_path: str, xtype: str, *, display: bool = False) -> str:
        ...
        if xtype == "p2wpkh":
            if coin_network == bitbox02.sfn.SFN:
                out_type = bitbox02.sfn.SFNPubRequest.ZPUB
            else:
                out_type = bitbox02.sfn.SFNPubRequest.VPUB
        elif xtype == "p2wpkh-p2sh":
            if coin_network == bitbox02.sfn.SFN:
                out_type = bitbox02.sfn.SFNPubRequest.YPUB
            else:
                out_type = bitbox02.sfn.SFNPubRequest.UPUB
        elif xtype == "p2wsh-p2sh":
            if coin_network == bitbox02.sfn.SFN:
                out_type = bitbox02.sfn.SFNPubRequest.CAPITAL_YPUB
            else:
                out_type = bitbox02.sfn.SFNPubRequest.CAPITAL_UPUB
        elif xtype == "p2wsh":
            if coin_network == bitbox02.sfn.SFN:
                out_type = bitbox02.sfn.SFNPubRequest.CAPITAL_ZPUB
            else:
                out_type = bitbox02.sfn.SFNPubRequest.CAPITAL_VPUB
        # The other legacy types are not supported
        else:
            raise Exception("invalid xtype:{}".format(xtype))

        return self.bitbox02_device.sfn_xpub(
    ...
    def sfn_multisig_config(
        ...
        multisig_config = bitbox02.sfn.SFNScriptConfig(
            multisig=bitbox02.sfn.SFNScriptConfig.Multisig(
                threshold=wallet.m,
                xpubs=[util.parse_xpub(xpub) for xpub in xpubs],
                our_xpub_index=xpubs.index(our_xpub),
                script_type={
                    "p2wsh": bitbox02.sfn.SFNScriptConfig.Multisig.P2WSH,
                    "p2wsh-p2sh": bitbox02.sfn.SFNScriptConfig.Multisig.P2WSH_P2SH,
                }[xtype]
            )
        )

        is_registered = self.bitbox02_device.sfn_is_script_config_registered(
            coin, multisig_config, account_keypath
        )
        if not is_registered:
            name = self.handler.name_multisig_account()
            try:
                self.bitbox02_device.sfn_register_script_config(
                    coin=coin,
    ...
    def show_address(
        ...
        if address_type == "p2wpkh":
            script_config = bitbox02.sfn.SFNScriptConfig(
                simple_type=bitbox02.sfn.SFNScriptConfig.P2WPKH
            )
        elif address_type == "p2wpkh-p2sh":
            script_config = bitbox02.sfn.SFNScriptConfig(
                simple_type=bitbox02.sfn.SFNScriptConfig.P2WPKH_P2SH
            )
        elif address_type in ("p2wsh-p2sh", "p2wsh"):
            if type(wallet) is Multisig_Wallet:
                script_config = self.sfn_multisig_config(
        ...
        return self.bitbox02_device.sfn_address(
    ...
    def _get_coin(self):
        return bitbox02.sfn.TSFN if constants.net.TESTNET else bitbox02.sfn.SFN
    ...
    def sign_transaction(
        ...
        for txin in tx.inputs():
            ...
            prev_inputs: List[bitbox02.SFNPrevTxInputType] = []
            prev_outputs: List[bitbox02.SFNPrevTxOutputType] = []
        ...
        if tx_script_type == "p2wpkh":
            tx_script_type = bitbox02.sfn.SFNScriptConfig(
                simple_type=bitbox02.sfn.SFNScriptConfig.P2WPKH
            )
            ...
            if type(wallet) is Multisig_Wallet:
                tx_script_type = self.sfn_multisig_config(coin, full_path, wallet, tx_script_type)
            else:
                raise Exception("Can only use p2wsh-p2sh or p2wsh with multisig wallets")
        else:
        ...
        sigs = self.bitbox02_device.sfn_sign(
            coin,
            [bitbox02.sfn.SFNScriptConfigWithKeypath(
    ...
    def sign_message(self, keypath: str, message: bytes, xtype: str) -> bytes:
        ...
        try:
            simple_type = {
                "p2wpkh-p2sh":bitbox02.sfn.SFNScriptConfig.P2WPKH_P2SH,
                "p2wpkh": bitbox02.sfn.SFNScriptConfig.P2WPKH,
            }[xtype]
        except KeyError:
            raise UserFacingException("The BitBox02 does not support signing messages for this address type: {}".format(xtype))

        _, _, signature = self.bitbox02_device.sfn_sign_msg(
            self._get_coin(),
            bitbox02.sfn.SFNScriptConfigWithKeypath(
                script_config=bitbox02.sfn.SFNScriptConfig(
