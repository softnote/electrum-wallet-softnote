...
class TrezorClientBase(HardwareClientBase, Logger):
    ...
    def get_xpub(self, bip32_path, xtype, creating=False):
        ...
        with self.run_flow(creating_wallet=creating):
            node = trezorlib.sfn.get_public_node(self.client, address_n).node
    ...
    def show_address(self, address_str, script_type, multisig=None):
        ...
        with self.run_flow():
            return trezorlib.sfn.get_address(
                self.client,
                coin_name,
                address_n,
                show_display=True,
    ...
    def sign_message(self, address_str, message):
        ...
        address_n = parse_path(address_str)
        with self.run_flow():
            return trezorlib.sfn.sign_message(
                self.client,
                coin_name,
                address_n,
                message)
    ...
    def sign_tx(self, *args, **kwargs):
        with self.run_flow():
            return trezorlib.sfn.sign_tx(self.client, *args, **kwargs)
    ...
    def get_passphrase(self, available_on_device):
        if self.creating_wallet:
            msg = _("Enter a passphrase to generate this wallet.  Each time "
                    ...
                    "access the softnotes in the wallet.").format(self.device)
        else:
