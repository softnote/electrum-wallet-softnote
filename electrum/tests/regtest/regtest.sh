...
softnote_cli="softnote-cli -rpcuser=doggman -rpcpassword=donkey -rpcport=18554 -regtest"

function new_blocks()
{
    $softnote_cli generatetoaddress $1 $($softnote_cli getnewaddress) > /dev/null
}
...
function wait_until_spent()
{
    ...
    while [[ $($softnote_cli gettxout $1 $2) ]]; do
        sleep 1
	msg="$msg."
	printf "$msg\r"
    done
...
if [[ $1 == "init" ]]; then
    ...
    if [[ $2 == "bob" ]]; then
        ...
        echo "funding $2"
        $softnote_cli sendtoaddress $($agent getunusedaddress -o) 1
    fi
fi
...
if [[ $1 == "breach" ]]; then
    ...
    $softnote_cli sendrawtransaction $ctx
    new_blocks 1
...
if [[ $1 == "breach_with_spent_htlc" ]]; then
    ...
    $bob stop
    ctx_id=$($softnote_cli sendrawtransaction $ctx)
    echo "alice breaches with old ctx:" $ctx_id
    new_blocks 1
    if [[ $($softnote_cli gettxout $ctx_id 0 | jq '.confirmations') != "1" ]]; then
