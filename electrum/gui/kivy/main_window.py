...
from electrum.util import (NoDynamicFeeEstimates, NotEnoughFunds,
                           SOFTNOTE_BIP21_URI_SCHEME, LIGHTNING_URI_SCHEME,
                           UserFacingException)
...
class ElectrumWindow(App, Logger):
    ...
    def on_new_intent(self, intent):
        ...
        if scheme == SOFTNOTE_BIP21_URI_SCHEME:
            self.set_URI(data)
    ...
    def sfn_to_fiat(self, amount_str):
        if not amount_str:
            return ''
    ...
    def fiat_to_sfn(self, fiat_amount):
        if not fiat_amount:
            return ''
        rate = self.fx.exchange_rate()
        if rate.is_nan():
    ...
    def on_qr(self, data: str):
        from electrum.softnote import is_address
        data = data.strip()
        if is_address(data):
            self.set_URI(data)
            return
    ...
    def on_start(self):
        ...
        # bind intent for softnote: URI scheme
        if platform == 'android':
            from android import activity
            from jnius import autoclass
            PythonActivity = autoclass('org.kivy.android.PythonActivity')
